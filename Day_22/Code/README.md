...
- **`utils/response.js`** — `sendSuccess()` / `sendError()` helpers so the JSON response shape is *enforced* by shared code, not just "consistent by convention" across files. Every controller, every validation middleware, and the error handler all funnel through these two functions.
...
**Error**
```json
{ "success": false, "error": "Validation failed", "details": ["price is required"] }
```
...
| **Products - Happy Paths** | full CRUD lifecycle: create → read → update (PATCH) → replace (PUT) → delete, in order, all expected to succeed |
...
| **CORS** | manual `OPTIONS` preflight check |
